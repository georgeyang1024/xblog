/**
 * marked - a markdown parser Copyright (c) 2011-2013, Christopher Jeffrey. (MIT
 * Licensed) https://github.com/chjj/marked
 */
;
(function() {
	var block = {
	...
	block.html = replace(block.html)('comment', /<!--[\s\S]*?-->/)('closed',
			/<(tag)[\s\S]+?<\/\1>/)('closing',
			/<tag(?:"[^"]*"|'[^']*'|[^'">])*?>/)(/tag/g, block._tag)();
	block.paragraph = replace(block.paragraph)('hr', block.hr)('heading',
			block.heading)('lheading', block.lheading)('blockquote',
	...
	inline.link = replace(inline.link)('inside', inline._inside)('href',
			inline._href)();
	...
	InlineLexer.prototype.outputLink = function(cap, link) {
		if (cap[0][0] !== '!') {
			return '<a href="' + escape(link.href) + '"'
					+ (link.title ? ' title="' + escape(link.title) + '"' : '')
					+ '>' + this.output(cap[1]) + '</a>';
		} else {
			return '<img src="' + escape(link.href) + '" alt="'
					+ escape(cap[1]) + '"'
					+ (link.title ? ' title="' + escape(link.title) + '"' : '')
					+ '>';
		}
	...
	Parser.prototype.tok = function() {
		...
		case 'heading': {
			return '<h' + this.token.depth + '>'
					+ this.inline.output(this.token.text) + '</h'
					+ this.token.depth + '>\n';
		}
		case 'code': {
			...
			return '<pre><code'
					+ (this.token.lang ? ' class="' + this.options.langPrefix
							+ this.token.lang + '"' : '') + '>'
...
(function() {
	...
	function sourceDecorator(options) {
		...
		fallthroughStylePatterns.push([ PR_LITERAL, /^@[a-z_$][a-z_$@0-9]*/i,
				...
				PR_LITERAL,
				new RegExp('^(?:' + '0x[a-f0-9]+'
						+ '|(?:\\d(?:_\\d+)*\\d*(?:\\.\\d*)?|\\.\\d\\+)'
						+ '(?:e[+\\-]?\\d+)?' + ')' + '[a-z]*', 'i'), null,
				'0123456789' ], [ PR_PLAIN, /^\\[\s\S]?/, null ], [
...
(function(window, document) {
	...
	var markdownEl = document.getElementsByTagName('xmp')[0]
			|| document.getElementsByTagName('textarea')[0], titleEl = document
			.getElementsByTagName('title0')[0], scriptEls = document
			.getElementsByTagName('script'), navbarEl = document
